...
public class MouseMoveGOd : MonoBehaviour
{
    ...
    void Update()
    {
        if(god_move.aaaa){
            rotationX=0f;
            rotationY= -90f-god_move.angle*Mathf.Rad2Deg;
        }
        else{
        ...
        rotationY += mouseMoveX * sesitivity * 0.02f; //* Time.deltaTime;
        // rotationY 변수의 값은 rotationY + (mouseMoveX * 마우스 민감도 * Time.deltaTime)
        
        rotationX += mouseMoveY * sesitivity * 0.02f; //* Time.deltaTime;
